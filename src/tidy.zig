...
const example_directories = [_][]const u8{"examples"};
...
const capability_gates = [_][]const u8{
    "capabilities.clipboard",
    "capabilities.monitor_query",
    "capabilities.remote",
    "capabilities.window_filter",
    "capabilities.window_targeted_input",
};

const ungated_exports = [_][]const u8{
    "pub const clipboard = plat" ++ "form.backend.clipboard;",
    "pub const message = plat" ++ "form.backend.message;",
    "pub const monitor = plat" ++ "form.backend.monitor;",
    "pub const remote = plat" ++ "form.backend.remote;",
    "pub const window = plat" ++ "form.backend.window;",
};
...
comptime {
    ...
    assert(capability_gates.len == ungated_exports.len);
...
fn tidy_capability_gates(errors: *Errors, sources: []const SourceFile) void {
    const root = find_source(sources, root_path) orelse {
        errors.add(root_path, 1, "root module is missing");

        return;
    };

    for (capability_gates) |gate| {
        if (std.mem.indexOf(u8, root.text, gate) != null) continue;

        errors.add_fmt(root.path, 1, "capability gate '{s}' is not declared", .{gate});
    }

    for (ungated_exports) |line| {
        const offset = std.mem.indexOf(u8, root.text, line) orelse continue;

        errors.add_fmt(root.path, line_of(root.text, offset), "export bypasses its gate", .{});
    }
}
...
test "tidy: sources obey the mechanical law" {
    ...
    tidy_capability_gates(&errors, sources);

    try tidy_extra_files(arena, io, &errors);
    try std.testing.expectEqual(@as(u32, 0), errors.count);
}

test "tidy: examples obey the same law as the sources" {
    const io = std.testing.io;
    const gpa = std.testing.allocator;
