...
pub fn SourceType(comptime Owner: type) type {
    return struct {
        ...
        pub fn install(instance: *Instance, owner: *Owner) Error!void {
            ...
            instance.handle = hook.Hook.install(.keyboard, trampoline, module);
        ...
        fn procedure(
            ...
            const parsed = event.parse_key(wparam, lparam) orelse
                return hook.next(code, wparam, lparam);
...
const Watcher = struct {
    pub fn process(_: *Watcher, _: *const Key) Response {
        return .pass;
    }
};

const Listener = struct {
    pub fn process(_: *Listener, _: *const Key) Response {
        return .pass;
    }
};

test "two owner types instantiate two distinct sources" {
    try testing.expect(SourceType(Watcher) != SourceType(Listener));
}

test "one keyboard hook slot is shared across every owner type" {
    instance_global.store(null, .seq_cst);
    procedure_global.store(null, .seq_cst);

    var watcher = Watcher{};
    var listener = Listener{};
    var second = SourceType(Listener).init();

    instance_global.store(&watcher, .seq_cst);

    try testing.expectError(Error.HookAlreadyInstalled, second.install(&listener));
    try testing.expect(second.handle == null);

    instance_global.store(null, .seq_cst);
}

test "removing a source that never installed leaves the slot alone" {
    instance_global.store(null, .seq_cst);

    var watcher = Watcher{};
    var source = SourceType(Watcher).init();

    instance_global.store(&watcher, .seq_cst);

    source.remove();

    try testing.expect(instance_global.load(.seq_cst) != null);
