...
pub const Button = enum(u8) {
    ...
    pub fn is_valid(button: Button) bool {
        const value = @intFromEnum(button);

        return value <= button_max;
    }

    pub fn down_flag(button: Button) u32 {
    ...
    pub fn up_flag(button: Button) u32 {
        assert(button.is_valid());
    ...
    pub fn xdata(button: Button) u32 {
        assert(button.is_valid());
...
pub const Input = extern struct {
    ...
    pub fn move_absolute(x: i32, y: i32) Input {
        ...
        assert(screen.virtual_width >= 0);
        assert(screen.virtual_height >= 0);
    ...
    pub fn wheel(delta: i32) Input {
        assert(delta != 0);
        assert(@rem(delta, wheel_delta) == 0);
        assert(delta >= -(wheel_delta * @as(i32, scroll_clicks_max)));
        assert(delta <= wheel_delta * @as(i32, scroll_clicks_max));
...
pub fn move_to_monitor(index: u8, x: i32, y: i32) bool {
    ...
    const position = monitor.to_absolute(x, y);
...
test "a relative mouse move carries its offset" {
    const event = Input.move(10, -5);
...
test "a wheel input encodes its delta" {
    const event = Input.wheel(wheel_delta);
...
test "a wheel input encodes a negative delta" {
    const event = Input.wheel(-wheel_delta);

    try testing.expectEqual(@as(u32, @bitCast(@as(i32, -120))), event.data.mouse.mouse_data);
}

test "a horizontal wheel input encodes its delta" {
    const event = Input.wheel_horizontal(wheel_delta);

    try testing.expectEqual(@as(u32, 120), event.data.mouse.mouse_data);
    try testing.expectEqual(@as(u32, 0x1000), event.data.mouse.flags);
}

test "an absolute mouse move is normalised into bounds" {
    const event = Input.move_absolute(0, 0);
...
test "a mouse button maps to its flags" {
    try testing.expectEqual(@as(u32, 0x0002), Button.left.down_flag());
    try testing.expectEqual(@as(u32, 0x0004), Button.left.up_flag());
    try testing.expectEqual(@as(u32, 0x0080), Button.x1.down_flag());
    try testing.expectEqual(@as(u32, 0x0001), Button.x1.xdata());
    try testing.expectEqual(@as(u32, 0x0002), Button.x2.xdata());
    try testing.expectEqual(@as(u32, 0), Button.middle.xdata());
}

test "mouse limits" {
    try testing.expect(scroll_clicks_max >= 1);
    try testing.expect(wheel_delta == 120);
}
